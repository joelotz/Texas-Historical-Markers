...
# coding: utf-8
"""
THC OSM Integration Toolkit
---------------------------

Utilities for reading, filtering, and synchronizing Texas Historical Commission
marker data with OpenStreetMap.

This script loads the Atlas CSV dataset, converts selected marker rows into
OSM-friendly node dictionaries with standardized tags, and can push those nodes
directly into JOSM via Remote Control for manual review or upload. It also
supports exporting modified atlas files back to CSV and includes helper
functions to identify markers missing from OSM based on a GeoJSON extract.

Main components:
    • read_atlas()      – Load the primary THC marker atlas CSV with column
                          datatypes predefined for consistency.
    • create_nodes()    – Convert marker rows into OSM node structures formatted
                          for import into JOSM. Builds tags including
                          memorial/plaque style, operator, website links, and
                          THC/HMDB reference fields.
    • push2josm()       – Sends nodes to a running JOSM instance using Remote
                          Control (port 8111). Returns the list of THC ref IDs
                          successfully added.
    • write2csv()       – Saves the atlas back to disk, optionally timestamped
                          for archival tracking.
    • find_missing_osm()– Compares atlas THC reference numbers with a GeoJSON
                          export from OSM to help locate markers not yet mapped.
    • update_isOSM()    – Marks rows in the atlas whose THC ref appears in a
                          provided list (typically output from push2josm()).

Intended workflow:
    Import atlas → generate nodes → push into JOSM → update local atlas fields →
    export updated dataset.

Useful for HMDB/OSM batch additions, catching unmapped THC markers, and
maintaining synchronized datasets between your personal atlas and OSM mapping
workflows.
"""
