...
| 7      | isActive                                                     | Boolean       | `False` marks a superseded or duplicate THC atlas record — the same physical marker is documented under another `ref:US-TX:thc`, so this row should be ignored by counts, exports and field-hunting lists. Renamed from `isTHC` on 2026-08-15: every row is a THC marker now, so the old name described nothing, and it had drifted into meaning two opposite things. |
